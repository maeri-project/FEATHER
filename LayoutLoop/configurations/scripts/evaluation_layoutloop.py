...
# arch_name = "edge_128.yaml"
arch_prefix = arch_name[:-5]
mapping_directory = "timeloop_search_edge_256/mapping_search"
########### Must Change
...
if __name__ == "__main__":
    ...
    for layout_policy in layout_policy_list:
        for model_name in model_name_list:
            for layer_id in range(1, layer_num[model_name]+1):
                ...
                command = f"timeloop-model ../arch_designs/{arch_name} ../{mapping_directory}/{model_name}_global_{layer_id}.yaml ../layer_shapes/{model_name}/{model_name}_layer{layer_id}.yaml ../layout/{model_name}/{arch_prefix}_{layout_policy}_{layer_id}.yaml"
                # command = f"timeloop-model ../arch_designs/{arch_name} ../{mapping_directory}/{model_name}_{layout_policy}_{layer_id}.yaml ../layer_shapes/{model_name}/{model_name}_layer{layer_id}.yaml ../layout/{model_name}/{arch_prefix}_{layout_policy}_{layer_id}.yaml"
                print(command)
                command_output = subprocess.check_output([f"{command}"], shell=True, universal_newlines=True)
    ...
    np.savetxt(os.path.join(work_directory, "interleaved_layoutloop_evaluation.csv"), interleave_overall_array, delimiter=',', fmt='%.2f')
