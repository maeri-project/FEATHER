...
map_policy_dict = {
    ...
    "sigma_transpose": "mapper/mapper_layout_loop.yaml",
...
map_constraint_dict = {
    ...
    "medusa": " ",
    "sigma_transpose": "", # Fully flexible dataflow space
    "systolic_array": " ",
...
arch_dict = {
    ...
    "sigma_transpose": "edge_256.yaml", # Fully flexible 1D array
    ...
    for layout_policy in layout_policy_list: 
        for model_name in model_name_list:
            for layer_id in range(1, layer_num[model_name]+1):
                ...
                command_output = subprocess.check_output(f"timeloop-mapper /home/jianming/work/lambda_workspace/arch_designs/{arch_dict[arch_prefix]} /home/jianming/work/lambda_workspace/{map_policy_dict[arch_prefix]} {map_constraint_dict[arch_prefix]} /home/jianming/work/lambda_workspace/layer_shapes/{model_name}/{model_name}_layer{layer_id}.yaml /home/jianming/work/lambda_workspace/layout/{model_name}/{arch_prefix}_{layer_id}.yaml", shell=True, universal_newlines=True)
                # command_output = subprocess.check_output([f"timeloop-mapper ../arch_designs/{arch_dict[arch_prefix]} {map_policy_dict[arch_prefix]} {map_constraint_dict[arch_prefix]} ../layer_shapes/{model_name}/{model_name}_layer{layer_id}.yaml ../layout/{model_name}/{arch_prefix}_{layout_policy}_{layer_id}.yaml"], shell=True, universal_newlines=True)
                # absolute path
